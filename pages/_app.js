...
MyApp.getInitialProps = async ({ Component, ctx }) => {
  const { nptl_users_token } = parseCookies(ctx);
  let pageProps = {};
  ...
  if (!nptl_users_token) {
    ...
    const ifLoggedIn =
      ctx.pathname === "/login" || ctx.pathname === "/reset-password";
    if (ifLoggedIn) {
      redirectUser(ctx, "/");
    }

    try {
      const payload = { headers: { Authorization: nptl_users_token } };
      const url = `${baseUrl}/api/users/update`;
      const response = await axios.get(url, payload);
      const user = response && response.data.user;

      if (!user) {
        destroyCookie(ctx, "nptl_users_token");
        redirectUser(ctx, "/login");
      }
      pageProps.user = user;
    } catch (err) {
